...
import com.exam.common.RedisBloomCacheConstant;
...
public class VideoServiceImpl implements VideoService {
    ...
    @Resource
    private RBloomFilter<String> videoLikeBloomFilter;
    ...
    public IPage<Video> getPublishedVideos(Integer page, Integer size, Long categoryId, String keyword, HttpServletRequest request) {
        ...
        if (request != null) {
            ...
            result.getRecords().forEach(video -> {
                ...
                String key = String.format(RedisBloomCacheConstant.VIDEO_LIKE_USER_IP, video.getId(), userIp);
                boolean isLiked = false;

                // 使用布隆过滤器判断点赞状态
                if (videoLikeBloomFilter.contains(key)) {
                    isLiked = true; // 可能已点赞
                } else {
                    isLiked = videoLikeMapper.isLikedByIp(video.getId(), userIp);
                    if (isLiked) {
                        videoLikeBloomFilter.add(key);
                    }
                }
    ...
    public boolean toggleVideoLike(Long videoId, HttpServletRequest request) {
        ...
        String userIp = IpUtils.getClientIp(request);
        String userAgent = request.getHeader("User-Agent");
        String key = String.format(RedisBloomCacheConstant.VIDEO_LIKE_USER_IP, videoId, userIp);

        // 检查是否已点赞
        boolean isLiked;

        // 1️⃣ 先从布隆过滤器判断是否可能已点赞
        if (videoLikeBloomFilter.contains(key)) {
            isLiked = true;
        } else {
            // 2️⃣ 未命中布隆过滤器，再查数据库确认
            isLiked = videoLikeMapper.isLikedByIp(videoId, userIp);
            // 若数据库存在点赞，补充进布隆过滤器
            if (isLiked) {
                videoLikeBloomFilter.add(key);
            }
